...
class waAPiClient:
    ...
    def register_for_event(self, contactId: int, eventName: str, regType: str) -> dict:
        ...
        if not eventType:
            return f"Event type '{regType}' not found for event '{eventName}'."
                                
        if not eventId or not eventType:
            return f"Event '{eventName}' or type '{regType}' not found."
    ...
    def get_event_registrations(self, eventName: str, limit: Optional[int] = 100, after: Optional[str] = None) -> dict:
        ...
        if not regs.json():
            return f"Event '{eventName}' not found or has no registrations."
